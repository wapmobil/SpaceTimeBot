...
class Battle {
	...
	buttons(chat_id, cont) {
		if (this.mode == -2) 
			return [{button: "Начать сражение!", script: "battle_start", data: `${this.players[0].nv.battle_id} 0`},
					{button: "Автобой", script: "battle_start", data: `${this.players[0].nv.battle_id} 2`},
					{button: "сбежать", script: "battle_start", data: `${this.players[0].nv.battle_id} 1`}];
		if (cont) 
			return [{button: "Продолжить сражение!", script: "battle_start", data: `${this.players[0].nv.battle_id} 0`},
					{button: "сбежать", script: "battle_start", data: `${this.players[0].nv.battle_id} 1`}];
		let a = [];
		let bt = [];
	...
	checkFinish() {
		if (!this.players[0].nv.battleList().some(e => e == 0) &&
			!this.players[1].nv.battleList().some(e => e == 0)) {this.finish(0); return;}
	...
	finish(side) {
		...
		if (side == 0) {
			...
			msg += "Ничья!\n Все проиграли."
			if (this.players[0].chat_id() > 1) {
				Telegram.edit(this.players[0].chat_id(), this.players[0].msg_id, msg);
			}
			if (this.players[1].chat_id() > 1)
				Telegram.edit(this.players[1].chat_id(), this.players[1].msg_id, msg);
		}
		if (side == 1) {
