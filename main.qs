...
if (isProduction) {
	...
	Telegram.start("1199643300:AAEN6IZzS4hCJTqyCXLMuaNvAXWiJiqG2LM");
}
...
function mining_info(chat_id){
		...
		Основная цель - добыча денег💰, но засчитывается только если дойти до выхода 🚪..\n
		Перемещение при помощи кнопок ◀️🔼🔽▶️ \n
		При нажатии кнопки бомбы 🧨 следующий переход взорвет стену 
		(или ничего если вы решили потратить бомбу на пустую клетку) \n
		Убийство монстров тоже приносит деньги, но отнимает здоровье 
		Крыса 🐀(-1) Летучая мышь 🦇(-2) Чужой 👽(-3)
