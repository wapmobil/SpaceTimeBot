...
function createSienceTree() {
	...
	s.addNext(new Research("🔍🔋Аккумуляторы", "Открывает доступ к строительству аккумуляторов", "enable_accum", 5000, 100000)).
	  addNext(new Research("🔍🔌Экономия энергии", "На 10% сокращает потребление электричества", "eco_power", 10000, 500000)).
	  add(new Research("🔍Налоги", "Добавляет +2💰 за каждый уровень базы", "enable_taxes", 80000, 1300000)).
	...
	s.addNext(new Research("🔍🛠Быстрое строительство", "В 2 раза ускоряет постройку зданий", "fastbuild", 4000, 1000000)).
	  addNext(new Research("🔍📦Компактное хранение", "Хранилище вмещает в 2 раза больше 💰", "upgrade_capacity", 5000, 1500000)).
	  addNext(new Research("🔍🛠Быстрое строительство 2", "На 50% ускоряет постройку зданий", "fastbuild", 10000, 2000000)).
	  addNext(new Research("🔍📦Компактное хранение 2", "Хранилище вмещает в 2 раза больше 💰", "upgrade_capacity", 15000, 2500000)).
	  addNext(new Research("🔍🛠Быстрое строительство 3", "На 30% ускоряет постройку зданий", "fastbuild", 18000, 4000000)).
	  addNext(new Research("🔍📦Компактное хранение 3", "Хранилище вмещает в 2 раза больше 💰", "upgrade_capacity", 20000, 5000000));
	return s;
}
